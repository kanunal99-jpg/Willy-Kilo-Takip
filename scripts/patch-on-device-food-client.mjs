...
if (!source.includes('On-device vision labels:')) {
  const functionRegex = /  const runAiAnalysis = async \(\) => \{[\s\S]*?\n  \};/;
  const replacement = [
  ...
  if (!functionRegex.test(source)) throw new Error('AddFoodModal runAiAnalysis function not found; refusing unsafe patch');
  source = source.replace(functionRegex, replacement);
}
