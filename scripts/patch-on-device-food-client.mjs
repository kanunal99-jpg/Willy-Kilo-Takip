...
const old = """  const runAiAnalysis = async () => {
...
const replacement = """  const runAiAnalysis = async () => {
    if (!imagePreview && !aiPromptNote) return;
    setIsAnalyzing(true); setAiError(null);
    try {
      let visionNote = '';
      ...
      if (json.success && json.data) {
        if (visionNote && json.provider === 'local') {
          json.data.note = `${json.data.note || ''} Cihaz içi ML Kit etiketleri: ${visionNote.replace('On-device vision labels: ', '')}`.trim();
