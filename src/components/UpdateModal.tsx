  ...
  const startUpdate = async () => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        await Browser.open({ url: data.apkUrl, toolbarColor: '#020617' });
      } else {
        const openedWindow = window.open(data.apkUrl, '_blank', 'noopener,noreferrer');
        if (!openedWindow) window.location.assign(data.apkUrl);
      }
      setOpened(true);
    } catch (e: any) {
      console.error('OTA APK open failed:', e);
      setStatus('error');
