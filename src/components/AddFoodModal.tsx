...
import { TURKISH_BAKERY_FOODS } from '../data/turkishBakeryCatalog';
...
const mergeFoodCatalog = (foods: FoodItem[]): FoodItem[] => {
  const map = new Map<string, FoodItem>();
  [...foods, ...TURKISH_BAKERY_FOODS].forEach((food) => map.set(food.id, food));
  return [...map.values()];
};
...
export const AddFoodModal: React.FC<AddFoodModalProps> = ({ isOpen, onClose, targetMeal, onAddFood }) => {
  ...
  const [foodDatabase, setFoodDatabase] = useState<FoodItem[]>(() => mergeFoodCatalog(loadFoods()));
  ...
  useEffect(() => { if (isOpen) setFoodDatabase(mergeFoodCatalog(loadFoods())); }, [isOpen]);
  ...
  const filteredFoods = foodDatabase.filter(f => {
    ...
    if (!q) return true;
    return normalizeSearch(f.name).includes(q) || normalizeSearch(f.category).includes(q) || (f.barcode && f.barcode.includes(searchQuery.trim())) || (f.pros && f.pros.some(p => normalizeSearch(p).includes(q)));
  });
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in">
      <div className="w-full max-w-lg rounded-t-3xl sm:rounded-3xl bg-slate-900 border border-slate-700 max-h-[92vh] flex flex-col shadow-2xl text-slate-100 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {activeTab === 'search' && <div className="space-y-4"><div className="relative"><Search className="w-4 h-4 absolute left-3.5 top-3.5 text-slate-400" /><input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Yumurta, tavuk, muz, çorba, poğaça, börek, simit ara..." className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-sm text-white" /></div>{selectedFood ? <div className="p-4 rounded-2xl bg-slate-950 border border-slate-700 space-y-4"><div className="flex justify-between"><div><span className="text-[10px] text-emerald-400 font-semibold uppercase">{selectedFood.category}</span><h4 className="text-base font-bold text-white">{selectedFood.name}</h4><p className="text-xs text-slate-400">{selectedFood.calories} kcal ({selectedFood.defaultServing} {selectedFood.unit})</p></div><button onClick={() => setSelectedFood(null)} className="text-xs text-slate-400">Geri</button></div><input type="number" min="0.25" step="0.25" value={servingAmount} onChange={e => setServingAmount(parseFloat(e.target.value) || 1)} className="w-24 px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-sm text-white font-bold" /><div className="p-3 rounded-xl bg-slate-900 text-xs">Toplam: <strong className="text-emerald-400">{Math.round(selectedFood.calories * servingAmount)} kcal</strong> · Prot: {Math.round(selectedFood.protein * servingAmount)}g · Karb: {Math.round(selectedFood.carbs * servingAmount)}g · Yağ: {Math.round(selectedFood.fat * servingAmount)}g</div><button onClick={() => confirmDbItem(selectedFood)} className="w-full py-2.5 rounded-xl bg-emerald-500 text-slate-950 font-bold text-sm"><Check className="w-4 h-4 inline mr-2" />Günlüğe Ekle</button></div> : <div className="space-y-2">{filteredFoods.length > 0 ? filteredFoods.map(food => <div key={food.id} onClick={() => { setSelectedFood(food); setServingAmount(1); }} className="p-3 rounded-xl bg-slate-950/60 hover:bg-slate-800/80 border border-slate-800 cursor-pointer flex items-center justify-between"><div><h5 className="text-sm font-semibold text-white">{food.name}</h5><p className="text-xs text-slate-400">{food.calories} kcal • P: {food.protein}g • K: {food.carbs}g • Y: {food.fat}g</p></div><Plus className="w-4 h-4 text-emerald-400" /></div>) : <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-center"><Search className="w-7 h-7 mx-auto text-slate-500" /><p className="text-sm font-bold text-white mt-2">Sonuç bulunamadı</p><p className="text-xs text-slate-400 mt-1">Farklı bir ad deneyin veya AI Tara ile yemeği analiz edin.</p></div>}</div>}</div>}
