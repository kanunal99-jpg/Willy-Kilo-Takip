    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in">
      <div className="w-full max-w-lg rounded-t-3xl sm:rounded-3xl bg-slate-900 border border-slate-700 max-h-[92vh] flex flex-col shadow-2xl text-slate-100 overflow-hidden">
        ...
        <div className="grid grid-cols-4 p-2 bg-slate-950/60 border-b border-slate-800 text-xs">
          {([['ai', Sparkles, 'AI Tara'], ['search', Search, 'Arama'], ['barcode', Barcode, 'Barkod'], ['custom', Plus, 'Manuel']] as const).map(([tab, Icon, label]) => <button key={tab} onClick={() => setActiveTab(tab)} className={`flex flex-col items-center gap-1 py-2 rounded-xl transition ${activeTab === tab ? 'bg-emerald-500 text-slate-950 font-bold shadow-md' : 'text-slate-400 hover:text-white'}`}><Icon className="w-4 h-4" /><span>{label}</span></button>)}
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {activeTab === 'barcode' && <div className="space-y-4"><div className="p-3 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 text-xs text-cyan-300"><strong className="text-white block">Barkod Tarama</strong>Paketli ürünlerin barkod numarasını girin.</div><div className="flex gap-2"><input type="text" value={barcodeInput} onChange={e => setBarcodeInput(e.target.value)} placeholder="Barkod girin" className="flex-1 px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-sm text-white" /><button onClick={() => handleBarcodeLookup(barcodeInput || '8690123456789')} className="px-4 py-2.5 rounded-xl bg-cyan-500 text-slate-950 font-bold text-xs">Sorgula</button></div>{barcodeResult && <div className="p-4 rounded-2xl bg-slate-950 border border-cyan-500/40 space-y-3"><h4 className="text-sm font-bold text-white">{barcodeResult.name}</h4><p className="text-xs text-slate-400">{barcodeResult.calories} kcal • 1 {barcodeResult.unit}</p><button onClick={() => confirmDbItem(barcodeResult)} className="w-full py-2.5 rounded-xl bg-cyan-500 text-slate-950 font-bold text-sm"><Check className="w-4 h-4 inline mr-2" />Günlüğe Ekle</button></div>}</div>}
