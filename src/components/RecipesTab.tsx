...
import {TURKISH_BAKERY_RECIPES} from '../data/turkishBakeryCatalog';
...
export const RecipesTab:React.FC<Props>=({recipes,onAddRecipeToDiary})=>{
 ...
 const mergedRecipes=useMemo(()=>{
  ...
  [...recipes,...TURKISH_BAKERY_RECIPES].forEach(recipe=>map.set(recipe.id,recipe));
  return [...map.values()];
 },[recipes]);
 return <div className="space-y-4">
  ...
  {view==='recipes'?<LegacyRecipesTab recipes={mergedRecipes} onAddRecipeToDiary={onAddRecipeToDiary}/>:<BeverageCatalog onAddRecipeToDiary={onAddRecipeToDiary}/>} 
 </div>;
};
