...
export const BeverageCatalog:React.FC<{onAddRecipeToDiary:(e:MealFoodEntry)=>void}>=({onAddRecipeToDiary})=>{
 ...
 return <div className="space-y-4">
  ...
  <div className="rounded-2xl bg-slate-900/80 border border-slate-800 p-3 space-y-3"><div className="flex justify-between text-sm font-bold"><span className="flex gap-2 items-center"><Filter className="w-4 h-4 text-emerald-400"/> Hızlı seçim</span><button onClick={clear} className="text-[11px] text-cyan-300">Temizle</button></div>
   ...
   <Facet title="Hedef">{(['Şekersiz','Düşük Kalori','Yüksek Protein','Diyet','Ferahlatıcı','Enerji Veren'] as Goal[]).map(x=><Chip key={x} onClick={()=>setGoal(goal===x?null:x)} active={goal===x}>{x}</Chip>)}</Facet>
   <Facet title="Kalori">{[[50,'0–50'],[100,'50–100'],[200,'100–200'],[10000,'200+']].map(([m,l])=><Chip key={l} onClick={()=>setCal(cal===m?null:m as number)} active={cal===m}>{l} kcal</Chip>)}</Facet>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{results.map(r=><button key={r.id} onClick={()=>setActive(r)} className="text-left rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden hover:border-cyan-500/40"><div className="relative h-40"><img src={r.imageUrl} alt={r.title} loading="lazy" className="w-full h-full object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-slate-950 to-transparent"/><span className="absolute top-2 left-2 px-2 py-1 rounded-full bg-emerald-400 text-slate-950 text-[10px] font-bold">ALKOLSÜZ</span><span className="absolute top-2 right-2 px-2 py-1 rounded-full bg-black/60 text-white text-[10px]">{vl(r.volumeMl)}</span><h3 className="absolute bottom-2 left-3 right-3 text-sm font-bold text-white">{r.title}</h3></div><div className="p-3 flex justify-between text-[11px]"><b className="text-emerald-400">{r.calories} kcal</b><span className="text-slate-400">P {r.protein}g • {r.prepTimeMinutes} dk</span><span className="text-cyan-300">Aç →</span></div></button>)}</div>
