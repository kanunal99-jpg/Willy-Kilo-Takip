...
import { Sparkles, Bot, Send, CheckCircle2, Award, Zap, ShieldCheck, Flame, MessageSquare, Loader2 } from 'lucide-react';
...
export const WillyCoachTab: React.FC<WillyCoachTabProps> = ({
  profile,
  todayData,
  activeFast,
  onUpdateProfile,
  ...
  // Calculate consumed calories & stats
  ...
  const quickQuestions = [
    'Bugün akşam yemeğinde ne yemeliyim?',
    'Kilo vermem yavaşladı, ne yapabilirim?',
    'Oruç penceresinde kahve içebilir miyim?',
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const botMsg = {
        sender: 'willy' as const,
        text: json.reply || 'Harika bir soru! Beslenme dengeni korumaya devam et.',
        time: new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }),
      };
      ...
      setMessages((prev) => [
        ...
        {
          sender: 'willy',
          text: 'Harika bir motivasyon! Su tüketimini ve protein oranını yüksek tutarak hedefine emin adımlarla ilerleyebilirsin.',
          time: 'Şimdi',
        },
      ]);
    } finally {
      ...
      <div className="rounded-3xl bg-gradient-to-br from-emerald-950/60 via-slate-900 to-cyan-950/40 border border-emerald-500/40 p-5 shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-4 pt-3 border-t border-slate-800/80">
          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center gap-2">
            ...
            <span className="text-[11px] text-slate-200 font-medium">Sınırsız AI Fotoğraf</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="text-[11px] text-slate-200 font-medium">Aralıklı Oruç (Tüm Planlar)</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="text-[11px] text-slate-200 font-medium">Akıllı Barkod Tarayıcı</span>
          </div>
          <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="text-[11px] text-slate-200 font-medium">Güvenli Bulut Eşitleme</span>
          </div>
        </div>
      </div>

      {/* AI Coach Live Chat Card */}
      <div className="rounded-3xl bg-slate-900 border border-slate-800 flex flex-col h-[460px] shadow-xl overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                Willy AI Beslenme Koçu
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              </h4>
              <p className="text-[10px] text-emerald-400">Çevrimiçi • Kişiye Özel Diyetisyen</p>
            </div>
          </div>
        </div>
              ...
              <div
                className={`max-w-[85%] p-3.5 rounded-2xl text-xs leading-relaxed ${
                  m.sender === 'user'
                    ...
                    : 'bg-slate-950 border border-slate-800 text-slate-100 rounded-tl-none shadow-sm'
                }`}
              >
                {m.text}
              </div>
        ...
        <div className="px-3 py-2 bg-slate-950/40 border-t border-slate-800 flex gap-1.5 overflow-x-auto">
          {quickQuestions.map((q, i) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-slate-800 hover:bg-slate-700 text-[10px] text-slate-300 whitespace-nowrap transition cursor-pointer"
            >
              {q}
            </button>
          ))}
        ...
        <div className="p-3 bg-slate-950 border-t border-slate-800 flex items-center gap-2">
          <input
            ...
            placeholder="Willy'ye beslenme veya kilon hakkında bir şey sor..."
            className="flex-1 px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
          />
          <button
            onClick={() => handleSendMessage()}
            disabled={isLoading || !inputText.trim()}
            className="p-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 disabled:opacity-40 transition shadow-md shadow-emerald-500/20 cursor-pointer"
          >
