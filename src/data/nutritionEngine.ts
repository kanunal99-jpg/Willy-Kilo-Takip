...
export function calculateNutrition(items: CompatibleIngredient[], servings = 1): NutritionResult {
  ...
  const entries = mapped.length ? mapped.map((x) => ({ key: x.key, name: x.name, kcalPer100g: x.kcalPer100g, proteinPer100g: x.proteinPer100g, carbsPer100g: x.carbsPer100g, fatPer100g: x.fatPer100g, fiberPer100g: x.fiberPer100g, source: x.source, sourceUrl: x.sourceUrl, confidence: x.confidence === 1 ? 'reference' as const : 'estimated' as const })) : [];
  ...
  return {
    ...
    source,
    confidence,
    provenance: { source, confidence, entries },
  };
}
